...
You are writing project-root AGENTS.md - the complete development guide for an entire project. This file lives at `<project_root>/AGENTS.md` (exact path match, not subdirectories).
...
**Public Knowledge = Information available in standard documentation, tutorials, or common practice:**
- Git basics ("git add, git commit, git push")
- Standard workflows ("create branch, make commits, create PR")
- Common practices ("Follow PEP 8", "Use semantic versioning")
- Tool explanations ("Git is a version control system...")

**Project-Specific = Information unique to THIS project:**
...
When creating or modifying project-root AGENTS.md:

1. Validate content against rules (public knowledge, required sections, structure)
2. Confirm severe violations with user (options A/B/C)
3. Fix deterministic issues silently (formatting, missing translation)
4. Write AGENTS.md in English
5. Synchronize AGENTS.zh-CN.md (incremental update or spawn translation subagent)
6. Suggest minor improvements after write (don't block)
...
| Adding new section | Validate necessity → Write EN → Update ZH (incremental) |
...
- "User knows their project best" (User may not know documentation rules)
- "I'll handle translation separately" (Translation must be synchronous)
...
- "This is different because..." (Special cases are rare; follow rules first)
- "Architecture explains what it does" ("What This Project Does" is REQUIRED regardless)
- "User only asked for X" (Implicit requirements like translation still apply)
- "Project-root can be longer" (True, but doesn't mean verbose; aim for conciseness)
- "More documentation is better" (Wrong. Relevant documentation is better)
...
## Writing Effective AGENTS Content

AGENTS.md is a prompt for AI agents. Apply prompt engineering principles:

### Structure Order

Organize each section following this pattern:

1. **Role/Context** — Set the scenario ("When working on X module...")
2. **Task** — Concrete actions ("Run these commands before commit")
3. **Constraints** — Boundaries ("Never commit without formatting")
4. **Output Format** — Expected results ("Tests must pass")
5. **Examples** — Show don't tell (code snippets, command examples)

### Core Principles

**Specific over abstract:**
- ✅ "Run `black {file}` before commit"
- ❌ "Format your code properly"

**Positive over negative:**
- ✅ "Use assert from cdef/debug.h"
- ❌ "Don't use <cassert>" (draws attention to forbidden thing)

**Show over tell:**
- ✅ Provide working command: `npm run test -- --coverage`
- ❌ Explain testing philosophy for 3 paragraphs

**Atomic instructions:**
- ✅ One instruction per bullet
- ❌ "Format code and run tests and check coverage while ensuring..."

**Constraint boundaries:**
- ✅ "Only modify files in src/core/"
- ❌ Unbounded: "Be careful when modifying files"

**Ordered by priority:**
- Put critical rules first (attention degrades over length)
- Format commands before optional style guidelines

### Common Pitfalls

**Contradictory instructions:**
- ❌ "Keep commits small" + "Include all related changes in one commit"
- ✅ Specify when each applies or pick one

**Implicit assumptions:**
- ❌ Assuming agents know your custom tool locations
- ✅ State explicitly: "Format script is in scripts/format.sh"

**Over-specification:**
- ❌ Documenting every possible edge case
- ✅ Cover common cases, let agents ask about edge cases

**Missing edge cases:**
- ❌ Only documenting happy path
- ✅ Include error handling: "If build fails, check X"

**Vague success criteria:**
- ❌ "Make sure code quality is good"
- ✅ "All tests pass, coverage >80%, no linter errors"

### Quick Check

Before finalizing AGENTS.md content, verify:
